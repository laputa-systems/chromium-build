#!/usr/bin/env python3
"""Run the fast browser tests after resolving the built Chrome binary."""
...
def main() -> None:
    ...
    output.chmod(0o777)
    for name in ("test-fast.json", "fast-screenshot.png"):
        (output / name).unlink(missing_ok=True)
    command = [
        "python3",
        str(root / "scripts/test-fast.py"),
        "--chrome",
        chrome,
        "--fixtures",
...
if __name__ == "__main__":
    raise SystemExit(fail_main("test-fast", main))
