...
def main() -> None:
    ...
    if llvm_root != Path("/opt/llvm-musl"):
        raise ScriptFailure("toolchain root is not /opt/llvm-musl")
    ...
    if run([str(ninja), "--version"], capture_output=True).stdout.strip() != "1.12.1":
    ...
    for path in (llvm_root / "include/c++/v1", llvm_root / "lib/libc++.a", llvm_root / "lib/libc++abi.a", llvm_root / "lib/libunwind.a"):
        if not path.exists():
            raise ScriptFailure(f"missing toolchain content {path}")
    ...
    if not list(resource_dir.glob("*/lib/linux/libclang_rt.builtins-*.a")) and not list(resource_dir.glob("lib/linux/libclang_rt.builtins-*.a")):
        raise ScriptFailure("missing compiler-rt builtins")
    if not list((llvm_root / "include/c++/v1").rglob("__config_site")):
        raise ScriptFailure("missing libc++ __config_site")
    ...
    namespace = f"{arch}-{arch_toolchain['archive_sha256']}"
    if os.environ.get("CCACHE_NAMESPACE") != namespace or os.environ.get("CCACHE_CC") != cc:
        raise ScriptFailure("ccache namespace or compiler is invalid")
    ...
    for text in ("compiler_check = content", "compression = true", f"namespace = {namespace}"):
